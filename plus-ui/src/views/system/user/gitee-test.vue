  ...
  <div class="p-4">
    ...
    <el-card class="mb-4">
      ...
      <el-form :inline="true">
        ...
        <el-form-item>
          <el-button type="primary" @click="testGiteeUserInfo" :loading="loading">
            获取用户信息
          </el-button>
        </el-form-item>
      </el-form>
    ...
    <el-card class="mb-4">
      ...
      <el-button type="success" @click="testContributions" :loading="loadingContributions">
        获取贡献记录
      </el-button>
      ...
      <div v-if="contributions.length > 0" class="mt-4">
        ...
        <div v-for="contribution in contributions.slice(0, 10)" :key="contribution.id" class="mb-2 p-2 border rounded">
          <div class="text-sm">
            <strong>类型:</strong> {{ contribution.type }}
          </div>
          <div class="text-sm">
            <strong>仓库:</strong> {{ contribution.repo?.name }}
          </div>
          <div class="text-sm">
            <strong>时间:</strong> {{ formatDate(contribution.created_at) }}
          </div>
        </div>
      </div>
    </el-card>
